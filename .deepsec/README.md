...
Currently configured project: `homelab-agent` (target: `..`).
...
3. Open the parent repo in your coding agent (Claude Code, Cursor, …)
   and have it follow `data/homelab-agent/SETUP.md` to fill in
   `data/homelab-agent/INFO.md`.
...
`--project-id homelab-agent` (or whichever id you want) explicitly.

`scan` is free (regex only). `process` is the AI stage (≈$0.30/file
on Opus by default). Run state goes to `data/homelab-agent/`.
